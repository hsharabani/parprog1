...
object ParallelCountChange {
  ...
  def parCountChange(money: Int, coins: List[Int], threshold: Threshold): Int = {
    if (threshold(money, coins) || money <= 0 || coins.isEmpty) {
      ...
      coins match {
        case head :: tail =>
          val (r1, r2) = parallel(parCountChange(money - head, coins, threshold), parCountChange(money, tail, threshold))
          r1 + r2
      }
    }
  }
  ...
  def combinedThreshold(startingMoney: Int, allCoins: List[Int]): Threshold = {
    (money, coins) => money * coins.length * 2 <= startingMoney * allCoins.length
  }
}
